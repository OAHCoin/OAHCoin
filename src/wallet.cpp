...
// oldagehomecoin: optional setting to unlock wallet for block minting only;
...
// oldagehomecoin: total coins staked (non-spendable until maturity)
...
bool CWallet::SelectCoinsMinConf(int64 nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, vector<COutput> vCoins, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet) const
{
    ...
    BOOST_FOREACH(COutput output, vCoins)
    {
        ...
        if (pcoin->nTime > nSpendTime)
            continue;  // oldagehomecoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while(true)
            {
                ...
                // oldagehomecoin: sub-cent change is moved to fee
...
// oldagehomecoin: create coin stake transaction
...
// oldagehomecoin: check 'spent' consistency between wallet and txindex
// oldagehomecoin: fix wallet spent state according to txindex
...
// oldagehomecoin: disable transaction (only for coinstake)
